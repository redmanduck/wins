...
for d in dirs:
    ...
    for f in files:
        ...
        for line in in_file:
            ...
            initial = data.get((vals[2], vals[3]), [0, 0, 0])
            data[(vals[2], vals[3])] = map(sum, zip(initial, (vals[0],
                    vals[1], 1)))
out_file = open('best_exps.txt', 'w')
num_types = len(dirs)
for f in maps:
    for k in maps[f].keys():
        if maps[f][k][2] != num_types:
            del maps[f][k]
    out_file.write(f + '\n')
    out_file.write('%5s %5s %5s %5s\n' % ('amean', 'astd', 'e1', 'e2'))
    l = sorted(maps[f].items(), key=lambda k: k[1])
    for v in l[:20]:
        out_file.write('%5.3f %5.3f %5.2f %5.2f\n' %
                (v[1][0] / num_types, v[1][1] / num_types, v[0][0], v[0][1]))
    out_file.write('\n')
